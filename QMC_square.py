...
from qmc_parameters import HamiltonianParams, QMCSampleData, QMCSamplingParameters
...
class QMC:
	...
	def __init__(self,EJ,EC,T,L,M):
		...
		### EC = Capacitive coupling in H_kin = EC * n^2
		### T = temperature
		...
		self.params = HamiltonianParams(EJ, EC, T, L, M)
		...
		self.Ky = self.params.Ky
		self.Kt = self.params.Kt ### The coupling between neighboring time slices
	...
	def draw_from_local_field(self,site):
		...
		if kappa < 1e-14: ### Safeguard for perfect cancellation
			...
			theta_out = self.rng.vonmises(mu=mu, kappa=kappa)
	...
	def set_sampling(self,nburn,nsample,nstep):
		self.sampling_params = QMCSamplingParameters(nburn, nsample, nstep)

		### Parameters relevant for MC sampling
		self.nburn = self.sampling_params.nburn  ### Number of burn steps. Should be updated at some point to a converging algorithm which runs until converged
		self.nsample = self.sampling_params.nsample ### How many samples we want
		self.nstep = self.sampling_params.nstep ### How many steps between each sample

		### TODO: Revisit this in the next major pass. vort_samples is fully derived from theta_samples
		### and can double the dominant storage cost for large production runs.
	...
	def get_sample_data(self):
		return QMCSampleData(self.action_samples, self.OP_samples, self.vort_samples, self.theta_samples)


### Compatibility with demler_tools
def run_MC_sims(save_filename,Ej,Ec,L,M,dt,nburn,nsample,nstep,hot_start_filename=None):
	params = HamiltonianParams.from_dt(Ej, Ec, L, M, dt)
	sim = QMC(params.EJ,params.EC,params.T,params.L,params.M)
	sim.over_relaxation = True
